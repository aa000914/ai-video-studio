...
const CHARACTERS = [
  {
    ...
    prohibited_changes: "玉佩必须始终在腰间、不可更换样式；布衣质感，不可出现丝绸；左眼角下方有一颗小痣；发型为秦式高髻，不可改为散发或披发",
  ...
  },
  {
    ...
    prohibited_changes: "王服必须为玄色（黑），不可改为其他颜色；冕冠必须端正、九旒；短须，不可出现长须或无须；两鬓微霜不可改为全黑；面容刚毅不需过度柔和化",
  ...
  },
  {
    ...
    prohibited_changes: "白须长髯，不可剪短或变色；紫竹杖不离手；腰间青铜小葫芦必须出现；面色红润不可苍白化",
